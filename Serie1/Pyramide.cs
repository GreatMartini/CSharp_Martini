...
namespace Martini_CSharp.Serie1
{
    ...
    {   public static void Construire(int N, bool s){
            ...
            // En fonction de N on a: gauche(j) = N - j - 2 et droite(j) = N + j - 2
            string[] etage;             //Declaration d'un etage
            etage = new string[2*N-1];
            int pos_g = N-2;              // Initialisation de la position de gauche
            int pos_d = N-2;              // Initialisation de la positiond de droite
            for(int i = 1; i < N; i++){
